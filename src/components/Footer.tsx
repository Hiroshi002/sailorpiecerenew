...
export default function Footer() {
  ...
  const footerLinks = siteConfig.nav.footer;
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex flex-col items-center text-center mb-16">
            ...
            <p className="max-w-2xl text-lg leading-relaxed italic text-gray-400 font-medium">
              ...
              <span className="opacity-90 font-bold text-[var(--accent-gold)]">{t.subtitle}</span>
            </p>
          </div>

          {/* Links Grid */}
          <nav className="flex flex-wrap justify-center gap-x-8 gap-y-4 mb-20 w-full max-w-4xl mx-auto">
            {footerLinks.map((link: { title: string; href: string }, i: number) => (
              <Link 
                key={i} 
                href={link.href} 
          ...
          <div className="w-full flex flex-col md:flex-row justify-between items-center gap-10">
            ...
            <div className="flex items-center order-2 md:order-1 border-l-4 border-[var(--accent-red)] bg-[#0a0a0a] py-3 px-8 shadow-[5px_5px_0px_rgba(255,30,56,0.2)]" style={{ transform: "skewX(-5deg)" }}>
              <div className="flex gap-6 items-center" style={{ transform: "skewX(5deg)" }}>
                {/* Roblox */}
                <a href={siteConfig.links.roblox} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white icon-hover-pulse transition-all duration-300">
                  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M5.156 18.156L3 11.375l6.781-2.156 2.156 6.781-6.781 2.156zm10.563-12.312l-6.781 2.156 2.156 6.781 6.781-2.156-2.156-6.781zM18.844 5.844L21 12.625l-6.781 2.156-2.156-6.781 6.781-2.156zM8.156 21.156l6.781-2.156-2.156-6.781-6.781 2.156 2.156 6.781z"/>
                  </svg>
                </a>
                {/* Discord */}
                <a href={siteConfig.links.discord} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#5865F2] hover:drop-shadow-[0_0_12px_rgba(88,101,242,0.6)] icon-hover-pulse transition-all duration-300">
                ...
                {/* Trello */}
                <a href={siteConfig.links.trello} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white icon-hover-pulse transition-all duration-300">
                  <svg className="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M19.7 3H4.3A1.3 1.3 0 003 4.3v15.4A1.3 1.3 0 004.3 21h15.4a1.3 1.3 0 001.3-1.3V4.3A1.3 1.3 0 0019.7 3zM9.7 17H5.3a.3.3 0 01-.3-.3V7.3a.3.3 0 01.3-.3h4.4a.3.3 0 01.3.3v9.4a.3.3 0 01-.3.3zm9.3-4h-4.4a.3.3 0 01-.3-.3V7.3a.3.3 0 01.3-.3H19a.3.3 0 01.3.3v5.4a.3.3 0 01-.3.3z"/>
                  </svg>
                </a>
                {/* YouTube */}
                <a href={siteConfig.links.youtube} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#FF0000] hover:drop-shadow-[0_0_12px_rgba(255,0,0,0.6)] icon-hover-pulse transition-all duration-300">
                  <svg className="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="flex flex-col items-center text-center order-1 md:order-2">
              <p className="text-xs uppercase tracking-[0.2em] font-black text-gray-400 mb-2">
                &copy; {new Date().getFullYear()} <span className="text-[var(--accent-red)]">{siteConfig.name}</span>
              </p>
